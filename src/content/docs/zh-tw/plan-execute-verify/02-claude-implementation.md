---
title: '計劃-執行-驗證：Claude SDK 實作'
description: '使用 Claude 的 Python SDK 建構生產級的 PEV 代理。包含約 1300 行經過實戰考驗的強健程式碼的完整教學。'
---

## 概覽

計劃-執行-驗證（Plan-Execute-Verify, PEV）模式將關注點分離為三個專門的元件，每個元件都針對其角色進行優化。這種架構比簡單的 [ReAct 迴圈](/ai-agent-study/zh-tw/react/01-overview/)強健得多，適合生產系統。
...
如果您只是在學習或建構簡單的工作流程，請參閱 [react-pattern.md](/ai-agent-study/zh-tw/react/01-overview/)。
...
我們還需要 `Plan`、`StepResult`、`VerificationResult` 和其他支援類別（請參閱下面的完整實作）。
...
完整的法律審查範例請參閱 [ai-agent-case-study.md](/ai-agent-study/zh-tw/blog/ai-agent-case-study/)。
...
| 功能         | ReAct 模式          | 計劃-執行-驗證              |
| ------------ | ------------------- | --------------------------- |
| **品質保證** | 無                  | 多階段驗證                  |
| **錯誤恢復** | 手動/以相同方式重試 | 帶回饋的智能重試 + 重新規劃 |
| **透明度**   | 一次一個動作        | 結構化計劃 + 進度追蹤       |
| **成本效率** | 可能浪費代幣        | 有針對性的重試、便宜的驗證  |
| **並行化**   | 僅順序              | 可以並行化獨立步驟          |
| **可觀測性** | 基本日誌            | 每個元件的豐富指標          |
| **生產就緒** | ❌ 否               | ✅ 是                       |
...
從 [ReAct](/ai-agent-study/zh-tw/react/01-overview/) 開始學習，在生產中使用 PEV。

## 相關資源

- [AI 代理案例研究](/ai-agent-study/zh-tw/blog/ai-agent-case-study/) - 法律審查範例
- [ReAct 模式](/ai-agent-study/zh-tw/react/01-overview/) - 更簡單的替代方案
